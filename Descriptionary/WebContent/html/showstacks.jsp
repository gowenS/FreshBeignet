...
<head>
	...
	<title>Fresh Beignet | Player <%=((int) session.getAttribute("stack_counter"))-1 %></title>
	<link rel="stylesheet" href="css/style.css">
  	<meta name="viewport" content="width=device-width, initial-scale=1.0">
</head>
<body>
	<h3>Progression for <%= (String) session.getAttribute("current_name") %></h3>
	<%String this_name;
	...
	<% } %>
	<%if(((int)session.getAttribute("stack_counter")-1) == ((int) session.getAttribute("numPlayers"))) { %>
		...
		<%} else { %>
			...
			<form action="show_stacks" method="get">
				<button class=button1>Show next player's stack.</button>
			</form>	
